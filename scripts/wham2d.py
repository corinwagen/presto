...
if args["type"] == "run":
    ...
    for file in files:
        for i, row in enumerate(coordinates):
            ...
            constraint_dict = {
                ...
                "atom2": args['x_atom2'],
                "equilibrium": float(x),
                "force_constant": args["x_force_constant"] * 0.0004184,
            }
            ...
            settings["constraints"]["wham_y"] = {
                ...
                "atom2": args['y_atom2'],
                "equilibrium": float(y),
                "force_constant": args["y_force_constant"] * 0.0004184,
            }
    ...
    def read_chk(file):
        ...
        for idx, frame in enumerate(traj.frames[args["cutoff"]:]):
            ...
            timeseries_text += f"{idx}\t{x_dist:.4f}\t{y_dist:.4f}\n"
        ...
        return x_file_hist, y_file_hist, len(dists), f"{name}.csv\t{dx:.4f}\t{dy:.4f}\t{kx:.4f}\t{ky:.4f}\n"
